    ...
    while 1:
        if(uavMkspn1 > truckMkspn1):
             ...
             if(maxSavings > 0):
                ...
                # print("now: uavMkspan, truckSpan，UAVassign and TruckAssign are:%6.2f" %(uavMkspn1))
                # print(truckMkspn1)
                # print(uavAssignments1)
                # print(truckRoute1)
...
if __name__ == "__main__":
    ...
    plt.plot(x, y, 'b', linewidth = 3)
    plt.text(Customers[truckRoute[0]-1].x_cor-0.2, Customers[truckRoute[0]-1].y_cor, str(truckRoute[0]), fontdict  = font_dict2)
    
    # conect depot to the last customer
    x = [data.cor_X[0], Customers[truckRoute[lengthTR-1]-1].x_cor]
    y = [data.cor_Y[0], Customers[truckRoute[lengthTR-1]-1].y_cor]    
    plt.plot(x, y, 'b', linewidth = 3)
    plt.text(Customers[truckRoute[lengthTR-1]-1].x_cor-0.2, Customers[truckRoute[lengthTR-1]-1].y_cor, str(truckRoute[lengthTR-1]), fontdict  = font_dict2)
